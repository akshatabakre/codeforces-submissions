...
void solve(){
    ...
    cin>>n;
    vector<int> p(n),c(n);
    vector<int> unsafe1(n,2),unsafe2(n,0);//from child, from parent
    for(int i=0;i<n;i++){
        cin>>p[i]>>c[i];
        if(p[i]!=-1){
            unsafe1[p[i]-1] = min(unsafe1[p[i]-1],c[i]);
        }
    }
    // for(int i=0;i<n;i++){
    //     if(p[i]!=-1){
    //         unsafe2[i] = max(unsafe2[i],c[p[i]-1]);
    //     }
    // }
    vector<int> unsafes;
    for(int i=0;i<n;i++){
        if(unsafe1[i]!=0 && c[i]){
            unsafes.push_back(i+1);
        }
    }
    // cout<<unsafe1[0]<<" "<<c[0]<<endl;
    if(unsafes.empty()){
        cout<<-1<<endl;
        return;
    }
    sort(unsafes.begin(),unsafes.end());
    for(int i:unsafes){
        cout<<i<<" ";
    }
    cout<<"\n";
